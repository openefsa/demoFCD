...
The European Food Safety Authority collects food consumption data from all EU countries and and integrates them in their database. The data is mainly based on country specific surveys, in which individuals get interviewed about their food consumption habits.
...
I selected for this tutorial the file of the "Chronic food consumption statistics" - "grams per day" - "Consumers only".

I downloaded the data and saved it in a file "chronicgdaytotpop_L4.csv"

The following code reads the data from the EFSA website. The file contains 4 sheets and we select the last one, which contains data on the lowest foodex level 4.
...
The food classification follows the Foodex standard developed by EFSA, which is a hierarchical system.
The current data contains the most detailed levels (3 and 4)

The 'Mean' column contains then the mean consumption in grams per day of this food among the participants of the survey. Additionally to the mean other statistics are given (standard deviation, percentiles)
...
One group of '308' 'adults' was surveyed and the mean food consumption of food 'Couscous' in that group was 1.28 g  intake per day. There are some more variables for the distribution of the daily intake.
